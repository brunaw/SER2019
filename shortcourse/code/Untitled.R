...
# Part 1. Authenticating vagalume and Spotify --------------------------
key_vagalume <- "c76fe6918c82ad8bdf96d80b517ced9b"

id <- "c6b3f40fc65047e68e6701f528fd3c30"
secret <- "5ee7fe63b5094b2195bed22be6c12e07"
app <- "my_app"
...
spot <- spot %>% 
  mutate(pop_class = ifelse(
    popul < quantile(popul, 0.33), "unpopular",
    ifelse(popul < quantile(popul, 0.66), "neutral", "popular")))
...
model_data <- feat_chords %>% 
  ...
  select(-analysis_url, -uri, -id.x, -id.y, -song, 
         ...
         -key.x, -song.id, 
         -dimi, -sus, # check this later
         -popul
         ) %>% 
  mutate(pop_class = as.factor(pop_class)) %>% 
  # Separate into train and test
  mutate(part = ifelse(runif(nrow(.)) > 0.25, 
                       "train", "test"))
...
# library(VSURF)
# sel_var <- VSURF(train %>% select(-pop_class),
#                           train %>% pull(pop_class))
# sel_var$varselect.pred
# names(train)[sel_var$varselect.thres]
# 
# form <- paste("pop_class ~ ", paste(names(train)[sel_var$varselect.thres],
#                                     collapse = " + ")) %>%
#      as.formula()
# m1 <- randomForest(form, data = train, mtry = 3)
# m1
