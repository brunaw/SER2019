...
- Uma forma de medir a distância entre os nomes é verificando se um nome é igual ao outro a menos de uma, duas ou três letras. Por exemplo:
...
Pega as palavras e seus passados.
...
- útil para guardar expressões mais complexas.
- a função `unnest_tokens()` é separa e empilha os n-gramas de cada letra.
...
all_data %>% 
  ...
  geom_point(colour = 'dodgerblue4', size = 3, alpha = 0.9) +
  coord_flip() +
  labs(x = 'Top 30 2-grams mais comuns', y = 'Counts') +
  theme_bw(14)
```
